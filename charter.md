...
|---------|------|----------|
| Build-id stamp | 2026-08-07 | Every build reported `0.0.1` in `--version` and in `invocation.binary_version`, because `alpha.yml` computed a channel tag and never passed it to the compiler (no `build.rs` existed). B3 says the audit trail is a feature; a record that cannot name the binary that wrote it undercuts that. Ported the stave/sidestep pattern: `bloomctl-sdk/build.rs` resolving `BLOOMCTL_BUILD_ID` → local `dev+g<sha7>[-dirty]` → `unknown`; `BUILD_ID`/`FULL_VERSION` consts with clap wired to the latter; additive `invocation.build_id` so `schema_version` stays at 2; both workflows stamping. Verified on the published alpha `alpha-20260807-050838-920be8d` after brew upgrade, not just locally. Audit lines predating this build carry no `build_id`, which identifies them as unstamped. 150 tests. GH #7 (closed), PR #8, bd `aae-orc-2586` (closed). Side-find: `dependency-review` has never passed here because Dependency graph is off; a fleet sweep showed bloomctl and stave are the only public repos missing it, both created private then flipped public. GH #9. |
