...
`bloomctl` is a CLI for two callers at once: a human at a terminal, and
...
Worked example: the tenant subdomain. It is the hostname component of
every single API call (`https://<subdomain>.api.kandji.io`), fixed for
the lifetime of an iru token. All four boxes check. Captured at
`auth login --subdomain`, resolved through a chain at Client
construction, the abuse never ships. (Third instantiation of the
pattern — sidestep's token chain and owner/customer chain are the
first two; see sidestep `aae-orc-y7lq`.)

## The fix — Argument Resolution Chain

For any near-constant value, walk:

1. **Flag** (`--subdomain <name>`) — explicit per-call override.
2. **Env** (`BLOOMCTL_SUBDOMAIN=<name>`) — per-shell or per-process default.
3. **Config** (`~/.config/bloomctl/config.toml [default] subdomain = ...`)
...
5. **Error**, with a message naming all four sources that failed and
   one concrete next step for each (`set BLOOMCTL_SUBDOMAIN=…`,
   `bloomctl config set subdomain …`).
...
Same chain for tokens, for the subdomain and region (B4), for the MCP
credentials (B6), for any future field that meets the abusive-argument
test. **One chain, not one per field.**
...
- **XDG-correct paths.** Config under `$XDG_CONFIG_HOME` (default
  `~/.config/bloomctl/`), state under `$XDG_STATE_HOME` (default
  `~/.local/state/bloomctl/`), cache under `$XDG_CACHE_HOME`. Never
  scatter dotfiles in `$HOME`.
- **Env over flags for invariants.** Anything a deployment would set
  once (token, subdomain, region, base URL) is reachable via env. Flags
...
uniformly and missed that one was constant. bloomctl inherited the rule
before its first keystroke: the subdomain (an even more constant value,
present on literally every call as the hostname) went straight into a
chain and never existed as a required flag.
...
- B4 (subdomain chain), B5 (write-guard), B7 (primitive layer) — `charter.md`
