//! JSON-line stream contract for bloomctl primitives.
//!
...
/// One record in a bloomctl JSON-line stream.
...
pub fn read_stream<R: BufRead>(reader: R) -> impl Iterator<Item = Result<Record>> {
    reader.lines().filter_map(|line_res| match line_res {
        Err(e) => Some(Err(BloomctlError::from(e))),
        Ok(line) if line.trim().is_empty() => None,
        Ok(line) => Some(serde_json::from_str::<Record>(&line).map_err(BloomctlError::from)),
    })
}
