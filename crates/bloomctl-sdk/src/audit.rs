...
/// globally disabled (`BLOOMCTL_AUDIT=off`).
pub fn audit_dir() -> Option<PathBuf> {
    if std::env::var("BLOOMCTL_AUDIT").is_ok_and(|v| v.eq_ignore_ascii_case("off")) {
        return None;
    }
    if let Ok(custom) = std::env::var("BLOOMCTL_AUDIT_DIR") {
        return Some(PathBuf::from(custom));
    }
    if let Some(state) = dirs::state_dir() {
        return Some(state.join("bloomctl").join("audit"));
    }
    if let Some(home) = dirs::home_dir() {
        return Some(home.join(".bloomctl").join("audit"));
    }
    None
}
