//! CEL adapter for `bloomctl filter --where '<CEL>'`.
//!
...
/// parse errors into [`BloomctlError::InvalidParam`].
pub fn compile(expression: &str) -> Result<Program> {
    Program::compile(expression)
        .map_err(|e| BloomctlError::InvalidParam("--where".into(), format!("CEL parse error: {e}")))
}
...
pub fn build_context(record: &Record, now: DateTime<Utc>) -> Result<Context<'static>> {
    ...
    ctx.add_variable("_source", &record.source)
        .map_err(|e| BloomctlError::InvalidParam("--where".into(), format!("bind _source: {e}")))?;
    ...
    record_map.push((
        ...
        cel_interpreter::objects::TryIntoValue::try_into_value(&record.source).map_err(|e| {
            BloomctlError::InvalidParam("--where".into(), format!("bind record._source: {e}"))
        })?,
    ));
            ...
            match parse_timestamp(value) {
                ...
                None => {
                    cel_interpreter::objects::TryIntoValue::try_into_value(value).map_err(|e| {
                        BloomctlError::InvalidParam("--where".into(), format!("bind `{name}`: {e}"))
                    })?
                }
            }
        } else {
...
/// [`BloomctlError::InvalidParam`] with the predicate text.
pub fn evaluate(
    program: &Program,
    record: &Record,
    now: DateTime<Utc>,
    ...
    match value {
        Value::Bool(b) => Ok(b),
        other => Err(BloomctlError::InvalidParam(
            "--where".into(),
...
mod tests {
    ...
    // BloomctlError::InvalidParam — not all malformed inputs are
