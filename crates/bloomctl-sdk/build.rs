...
//!
//! Resolution order:
//! 1. `BLOOMCTL_BUILD_ID` env — set by CI to the channel tag computed
...
fn main() {
    println!("cargo:rerun-if-env-changed=BLOOMCTL_BUILD_ID");
    ...
    let build_id = std::env::var("BLOOMCTL_BUILD_ID")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or_else(local_git_id);
    println!("cargo:rustc-env=BLOOMCTL_BUILD_ID={build_id}");
}
