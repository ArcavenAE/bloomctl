...
//! `BLOOMCTL_AUDIT_DIR` so they don't pollute the user's real trail.
...
fn tempdir() -> PathBuf {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let dir =
        std::env::temp_dir().join(format!("bloomctl-audit-{}-{n}-{nanos}", std::process::id()));
...
fn audit_can_be_disabled_with_off_env() {
    ...
    let lines = read_audit_lines(&dir);
    assert_eq!(lines.len(), 0, "BLOOMCTL_AUDIT=off must silence emission");
...
fn pipeline_filter_then_enrich_emits_two_verb_lines() {
    ...
    filter.env("BLOOMCTL_AUDIT_DIR", &dir);
    ...
    enrich.env("BLOOMCTL_AUDIT_DIR", &dir);
