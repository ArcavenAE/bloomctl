...
1. Capture every API call bloomctl makes — curated verb or `bloomctl api`
...
- Default: `~/.local/state/bloomctl/audit/YYYY-MM-DD.jsonl` (XDG state).
- Fallback when XDG is unset: `~/.bloomctl/audit/YYYY-MM-DD.jsonl`.
- Override: `BLOOMCTL_AUDIT_DIR` environment variable.
- Disable per-call: `--no-audit` flag (writes a stub line — see below).
- Disable globally: `BLOOMCTL_AUDIT=off`.

Files roll at UTC midnight. Old files are not pruned by bloomctl — that
...
- **User-defined extra paths:** `~/.config/bloomctl/redaction.toml` adds
...
`bloomctl --no-audit <op>` skips the operation-level detail but still
writes:
...
```json
{
  ...
  "ts_start": "...",
  "duration_ms": 187,
  "invocation": { "argv": ["bloomctl", "--no-audit", "..."], "binary_version": "0.1.0" },
...
`BLOOMCTL_AUDIT=off` writes nothing at all.

## Future analysis surface

Out of scope for v0.1. Charter F3 tracks this. Likely shapes:

- `bloomctl audit query --since 7d --group-by operation.id` — local
  aggregation.
- `bloomctl audit traces` — render trace_id trees.
- `bloomctl meta propose` — LLM call against a window of trail to
